...
export default function ListRapportsT() {

    const [documents, setDocuments] = useState([]);
    const [pdfFiles, setPdfFiles] = useState([]);

    const idTeacher = parseInt(localStorage.getItem('auth_id'));
    const { id } = useParams();
    //get files sent to student
    useEffect(() => {
        axios.get(`/api/Rapports_teacher?student=${id}&teacher=${idTeacher}`).then(res => {
    ...
    useEffect(() => {
        axios.get(`/api/Rapport_student?student=${id}`).then(res => {
            if (res.data.status === 200) {
    ...
    return (
        <div className="container">
            {/* <h6 style={style}> Documents envoyés par moi</h6>
            <hr />
            <div className="row">
                {files.length > 0 ? (
            ...
            <h6 style={style}> Documents envoyés par l'étudiant</h6>
            <hr />
            <div className="row">
                {files.length > 0 ? (
                    files.map((pdfFile, index) => (
